...
export default function BlockedPage() {
  ...
  return (
    <div style={{ maxWidth: 560, margin: '0 auto', padding: '32px 24px' }}>
      ...
      {loading ? (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 48, fontFamily: MONO, fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.25em' }}>LOADING...</div>
      ) : blocks.length === 0 ? (
        <p style={{ fontFamily: MONO, fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.15em', textAlign: 'center', padding: '48px 0' }}>No blocked members.</p>
      ) : (
            ...
            return (
              <div key={uid} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 0', borderBottom: '1px solid #111' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontFamily: MONO, fontSize: 8, color: 'rgba(255,255,255,0.4)' }}>@{user?.username}</p>
                </div>
                <button
                  onClick={() => unblock(uid)}
                  disabled={unbocking === uid}
                  style={{ padding: '6px 14px', background: 'transparent', border: '1px solid #333', color: 'rgba(255,255,255,0.4)', fontFamily: MONO, fontSize: 8, letterSpacing: '0.1em', cursor: 'pointer' }}>
