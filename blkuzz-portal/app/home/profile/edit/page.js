    ...
    <div className="page-fixed-shell" style={{ position: 'fixed', top: 0, left: 240, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', background: '#0a0a0a', color: '#e8e8e8', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderBottom: '1px solid #1a1a1a', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => router.back()} style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, letterSpacing: '0.15em', padding: '8px 18px', background: 'transparent', color: '#777', border: '1px solid #777', fontWeight: 700, cursor: 'pointer', textTransform: 'uppercase', borderRadius: 9999 }}>
            Cancel
          </button>
            ...
            {availableTags.map(tag => {
              ...
              return (
                <button key={tag} disabled={atMax}
                  ...
                  style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 9, letterSpacing: '0.1em', padding: '6px 12px', border: `1px solid ${active ? '#FDC214' : '#777'}`, color: active ? '#FDC214' : '#777', cursor: atMax ? 'not-allowed' : 'pointer', opacity: atMax ? 0.4 : 1, textTransform: 'uppercase', background: 'transparent', transition: 'all 0.1s', borderRadius: 9999 }}>
                  {tag}
                </button>
              )
            })}
